 ...
 * - 오른쪽: 펫 + 스페인어 (클릭으로 전환)
 */
class TravelPhraseSpanishWidgetProvider : AppWidgetProvider() {

    companion object {
        private const val ACTION_RIGHT_CLICK = "com.moveoftoday.walkorwait.TRAVEL_PHRASE_ES_RIGHT_CLICK"
        private const val PREFS_NAME = "TravelPhraseSpanishWidget"
        ...
        private const val LANG_CODE = "ES"

        fun updateAllWidgets(context: Context) {
            val appWidgetManager = AppWidgetManager.getInstance(context)
            val appWidgetIds = appWidgetManager.getAppWidgetIds(
                android.content.ComponentName(context, TravelPhraseSpanishWidgetProvider::class.java)
            )
        ...
        private fun updateWidget(
            ...
            // 오른쪽 영역 (스페인어는 로마자 언어이므로 state 1에서 바로 다음 문장)
            when (state) {
                0 -> {
                    if (showTapToday) {
            ...
            // 오른쪽 클릭
            val rightClickIntent = Intent(context, TravelPhraseSpanishWidgetProvider::class.java).apply {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == ACTION_RIGHT_CLICK) {
            ...
            if (appWidgetId != AppWidgetManager.INVALID_APPWIDGET_ID) {
                ...
                // 스페인어는 로마자 언어이므로 발음 표시 불필요 (state 1에서 바로 다음 문장)
                if (state > 1) {
                    state = 0
